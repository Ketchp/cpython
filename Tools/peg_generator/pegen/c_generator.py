...
class CParserGenerator(ParserGenerator, GrammarVisitor):
    def __init__(
        ...
        self.cleanup_statements: List[str] = []
        self._first = FirstFollow(self.all_rules, self.tokens, self.soft_keywords, only_valid=True)
        self._rule_descent: Dict[str, Any] = {}
    ...
    def _refill_token(self) -> None:
        if 'refiled' in self._rule_descent:
            return
        ...
        with self.indent():
            ...
            self.add_return("NULL")
        self.print("}")
        self._rule_descent['refiled'] = True
    ...
    def _extract_token_type(self) -> None:
        self._refill_token()
        self.print("int _type = p->tokens[_mark]->type;")
    ...
    def _handle_default_rule_body(self, node: Rule, rhs: Rhs, result_type: str) -> None:
        ...
        with self.indent():
            ...
            if memoize:
                ...
                self.print("}")
            self.print("int _mark = p->mark;")
            if node.name in self._first.ll1_decidable:
                self._extract_token_type()
    ...
    def visit_Rhs(
        ...
        for idx, alt in enumerate(node.alts):
            self.visit(alt, is_loop=is_loop, is_gather=is_gather, rulename=rulename, idx=idx)
    ...
    def _type_compare(self, token: str) -> str:
        if token in self.soft_keywords:
            return f'(_type == NAME && _PyPegen_peek_soft_keyword(p, {token}))'
        return f'_type == {self.callmakervisitor.name_to_macro(token)}'

    def _tokens_if_dipatch(self, tokens: Set[str], call_invalid: bool) -> None:
        if 'NAME' in tokens:
            tokens -= self.soft_keywords
        calls = [self._type_compare(token) for token in tokens]
        if call_invalid:
            calls.append("p->call_invalid_rules")
        if not calls:
            self.print("if(0)  // alt not directly reachable")
            return
        self.print("if (")
        with self.indent():
            for call in calls[:-1]:
    ...
    def visit_Alt(
        self, node: Alt, is_loop: bool, is_gather: bool, rulename: Optional[str], idx: int
    ) -> None:
        if rulename in self._first.ll1_decidable:
            tokens = self._first.alt_jumps[rulename][idx][idx]
            self._tokens_if_dipatch(tokens, call_invalid=(idx == 0))
            if idx == 0:
                self.print(f"{{  // {node}")
            else:
                self.print(f"{{ alt{idx}:  // {node}")
        elif any(str(item).startswith("invalid_") for item in node):
        ...
        with self.indent():
            ...
            if "_cut_var" in vars:
                ...
                self.print("}")
            if rulename in self._first.ll1_decidable:
                jumps = self._first.alt_jumps[rulename]
                if idx != len(jumps) - 1:
                    self.print(f"if (p->call_invalid_rules) goto alt{idx+1};")
                for dest, tokens in enumerate(jumps[idx]):
                    if not tokens or dest == idx:
                        continue
                    self._tokens_if_dipatch(tokens, call_invalid=False)
                    self.print(f"goto alt{dest};")
...
class FirstFollow:
    ...
    def __init__(self,
                 ...
                 only_valid: bool) -> None:
        self.rules = rules
        self.tokens = tokens
        self.soft_keywords = soft_keywords
        self.only_valid = only_valid
        self.alt_first: Dict[str, List[Set[str]]] = {}
        self.rule_follow: Dict[str, Set[str]] = {}
        self.ll1_decidable: set[str] = set()
        self.alt_jumps: Dict[str, List[List[Set[str]]]] = {}

    @property
    def rule_first(self) -> Dict[str, Set[str]]:
        return {rulename: set().union(*firsts)
                for rulename, firsts in self.alt_first.items()}

    def compute(self) -> None:
        self.alt_first = {
            name: [set() for alt in rule.rhs] for name, rule in self.rules.items()
        }
        self.rule_follow = {name: set() for name in self.rules}
        fields = [self.alt_first, self.rule_follow]
        ...
        for rulename, alts in self.alt_first.items():
            if rulename.startswith('_loop'):
                continue

            # switch speedup not needed for 'invalid_' rules
            if rulename.startswith('invalid_'):
                continue

            # todo: assert rules not nullable or implement follow

            # rule has only one path if not counting 'invalid_' alternatives
            if sum(e != set() for e in alts) < 2:
                continue

            # set at jumps[i][j] are tokens leading from 'alt i' to 'alt j'
            # for i==j, tokens in 'if' statement before alternative
            jumps: List[List[set[str]]] = [[set() for i in alts] for i in alts]
            last_seen: Dict[str, int] = {}
            seen: Set[str] = set()
            for j, alt in enumerate(alts):
                for token in alt:
                    i = last_seen.get(token, j)
                    jumps[i][j].add(token)
                    last_seen[token] = j
                    # jump from unmatched NAME to soft_keyword
                    if token in self.soft_keywords and 'NAME' in last_seen:
                        i = last_seen['NAME']
                        jumps[i][j].add(token)
                    # jump from unmatched soft_keyword to NAME
                    if token == 'NAME':
                ...
                if alt & seen:
                    # todo
                    # non_trivial[rulename] = alts
                    break
                seen |= alt
            else:
                if rulename.startswith('_loop'):
                    print(rulename)
                self.ll1_decidable.add(rulename)
            self.alt_jumps[rulename] = jumps

    def _single_pass(self) -> None:
        for rulename, rule in self.rules.items():
            if rulename.startswith("_loop"):
                assert len(rule.rhs.alts) == 1
                first_set = self.alt_first[rulename][0]
                if rulename.startswith("_loop0"):
                    first_set.add(self.epsilon)
                first_set |= self._process_alt(rule.rhs.alts[0])
                continue

            for alt_idx, alt in enumerate(rule.rhs):
                self.alt_first[rulename][alt_idx] |= self._process_alt(alt)

    def _process_alt(self, alt: Alt) -> Set[str]:
        if any(str(item.item).startswith("invalid_") for item in alt):
            return set()

        first: Set[str] = set()
        for idx, item in enumerate(alt):
            item_first = self._process_item(item.item)
            first |= item_first
            if self.epsilon in item_first:
    ...
    def _process_item(self, item: Item) -> Set[str]:
        # Item = Union[Leaf, Group, Opt, Repeat, Forced, Lookahead, Rhs, Cut]
        if isinstance(item, NameLeaf):
            if item.value in self.tokens:
                return {item.value}
            return self.rule_first[item.value]
        elif isinstance(item, StringLeaf):
            return {item.value}
        elif isinstance(item, Opt):
            return self._process_item(item.node) | {self.epsilon}
        elif isinstance(item, Forced):
            return self._process_item(item.node)
        elif isinstance(item, PositiveLookahead):
            return self._process_item(item.node)
        elif isinstance(item, NegativeLookahead):
            return {self.epsilon}  # todo: verify
        elif isinstance(item, Rhs):
            return set().union(*(self._process_alt(alt) for alt in item.alts))
        elif isinstance(item, Cut):
            return {self.epsilon}
        elif isinstance(item, Group) or isinstance(item, Repeat):
            print(f"Warning: processed grammar should not contain {type(item).__name__}")
            if isinstance(item, Group):
                return self._process_item(item.rhs)
            first = self._process_item(item.node)
            if isinstance(item, Gather) and self.epsilon in first:
                first |= self._process_item(item.separator)
            if isinstance(item, Repeat0):
                first.add(self.epsilon)
            return first
        else:
            raise ValueError(
                "FirstFollow._process_item expect union type Item,"
                f"got: {type(item)}"
            )
